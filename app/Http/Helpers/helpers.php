...
function agentOnBaseCurrency($profits){
    ...
    foreach ($profits as $profit) {
        $requestAmount = get_amount($profit->total_charge,null,get_wallet_precision());
        if($profit->transactions->type == PaymentGatewayConst::TYPEADDMONEY){
            $exchange_rate = get_amount($profit->transactions->currency->rate ?? get_default_currency_rate(),null,get_wallet_precision());
        }else{
            $exchange_rate = get_amount($profit->transactions->creator_wallet->currency->rate ?? get_default_currency_rate(),null,get_wallet_precision());
        }
        $result = get_amount($requestAmount / $exchange_rate, null, get_wallet_precision());
        $totalAmount += $result;
    }
    return $totalAmount??0 ;
}
...
function support_currencies(array $supported_currency) {
    if(count($supported_currency) > 0){
        ...
        $filtered_currency->makeHidden(['updated_at','admin_id','editData','sender','both','receiver','senderCurrency','receiverCurrency']);
        return $filtered_currency->values()??[];
    }else{
        $all_currency = Currency::active()->get();
        $all_currency->makeHidden(['updated_at','admin_id','editData','sender','both','receiver','senderCurrency','receiverCurrency']);
...
function user_wallets($guard,$column,$limit = null){
    ...
    if(is_null($limit)){
        if( $guard_name == 'api'){
            $user_wallets =  UserWallet::where($column,$user->id)->active()->with("currency:id,code,rate,flag,symbol,type,default,country")->get();
        }else if($guard_name == 'merchant_api'){
            $user_wallets =  MerchantWallet::where($column,$user->id)->active()->with("currency:id,code,rate,flag,symbol,type,default,country")->get();
        }elseif($guard_name == 'agent_api'){
            $user_wallets =  AgentWallet::where($column,$user->id)->active()->with("currency:id,code,rate,flag,symbol,type,default,country")->get();
        }else{
            $user_wallets = [];
        }
    }else{
        if( $guard_name == 'api'){
            $user_wallets =  UserWallet::where($column,$user->id)->active()->with("currency:id,code,rate,flag,symbol,type,default,country")->limit($limit)->get();
        }else if($guard_name == 'merchant_api'){
            $user_wallets =  MerchantWallet::where($column,$user->id)->active()->with("currency:id,code,rate,flag,symbol,type,default,country")->limit($limit)->get();
        }elseif($guard_name == 'agent_api'){
            $user_wallets =  AgentWallet::where($column,$user->id)->active()->with("currency:id,code,rate,flag,symbol,type,default,country")->limit($limit)->get();
        }else{
            $user_wallets = [];
        }
    }
    $user_wallets->makeHidden(['id','user_id','currency_id','created_at','updated_at','merchant_id','agent_id']);
    return $user_wallets??[];
}
