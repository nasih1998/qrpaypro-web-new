        ...
        <div class="col-xl-4 col-lg-4 col-md-6 form-group">
            ...
            <select name="country" class="form--control country-select select2-basic " data-minimum-results-for-search="Infinity">
                ...
                @foreach ($countries as $item)
                    <option value="{{ $item->id }}" {{ $item->id == @$data->country?'selected':'' }} data-country-code="{{ $item->code }}" data-mobile-code="{{ get_country_phone_code($item->country)}}"  data-id="{{ $item->id }}" data-country="{{ $item->country }}">{{ $item->name }} ({{ $item->code }})</option>
                @endforeach
            </select>
        </div>
        <div class="col-xl-4 col-lg-4 col-md-6 form-group">
