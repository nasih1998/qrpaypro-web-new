            ...
            <div class="row align-items-center mb-10-none">
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list-two">
                        ...
                        <li class="five">{{ __("Payable Unit Price")}}: <span>{{ get_amount(@$data->details->charge_info->sender_unit_price,@$data->details->charge_info->wallet_currency,get_wallet_precision($data->creator_wallet->currency))}}</span></li>
                ...
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list two">
                        <li class="one">{{ __("Exchange Rate")}}: <span>{{ get_amount(1,@$data->details->charge_info->card_currency) ." = ". get_amount(@$data->details->charge_info->exchange_rate,@$data->details->charge_info->wallet_currency,get_wallet_precision($data->creator_wallet->currency))}}</span></li>
                        <li class="two">{{ __("Card Quantity")}}: <span>{{ @$data->details->charge_info->qty}}</span></li>
                        <li class="three">{{ __("Total Charge")}}: <span>{{ get_amount(@$data->charge->total_charge,@$data->details->charge_info->wallet_currency,get_wallet_precision($data->creator_wallet->currency))}}</span></li>
                        <li class="four">{{ __("Payable Amount")}}: <span>{{ get_amount(@$data->payable,@$data->details->charge_info->wallet_currency,get_wallet_precision($data->creator_wallet->currency))}}</span></li>
