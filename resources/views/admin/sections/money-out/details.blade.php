            ...
            <div class="row align-items-center mb-10-none">
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list-two">
                        ...
                        <li class="five">{{ __("request Amount") }}: <span>{{ get_amount(@$data->request_amount,withdrawCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span></li>
                    </ul>
                </div>
                ...
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list two">
                        <li class="one">{{ __("Rate") }}: <span>1 {{ withdrawCurrency($data)['wallet_currency'] }} = {{ get_amount($data->details->charges->exchange_rate??$data->currency->rate,withdrawCurrency($data)['gateway_currency'],$precision) }} </span></li>

                        <li class="two">{{ __("Will Get") }}: <span>{{ get_amount(@$data->payable,withdrawCurrency($data)['gateway_currency'],$precision) }} </span></li>
                        <li class="three">{{ __("Total Charge") }}: <span>{{ get_amount($data->charge->total_charge??0,withdrawCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency))}}</span></li>
                        <li class="four">{{ __("Total Payable") }} : <span>{{ get_amount($data->details->charges->payable??$data->request_amount,withdrawCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span></li>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header p-3" id="approvedModalLabel">
                <h5 class="modal-title">{{ __("Approved Confirmation") }} ( <span class="fw-bold text-success">{{ get_amount(@$data->request_amount,withdrawCurrency($data)['wallet_currency'],$precision) }}</span> )</h5>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header p-3" id="rejectModalLabel">
                <h5 class="modal-title">{{ __("Rejection Confirmation") }} ( <span class="fw-bold text-danger">{{ get_amount(@$data->request_amount,withdrawCurrency($data)['wallet_currency'],$precision) }}</span> )</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
