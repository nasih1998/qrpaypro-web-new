            ...
            <div class="row align-items-center mb-10-none">
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list-two">
                        ...
                        <li class="five">{{ __("Bill Amount") }}: <span>{{ get_amount($data->request_amount,billPayCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span></li>
                ...
                <div class="col-xl-4 col-lg-4 form-group">
                    <ul class="user-profile-list two">
                        ...
                        <li class="two">{{ __("Total Charge") }}: <span>{{ get_amount($data->charge->total_charge,billPayCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span></li>
                        <li class="three">{{ __("Payable Amount") }}: <span>{{ get_amount($data->payable,billPayCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span></li>
                        <li class="three">{{ __("Conversion Balance") }}: <span>{{ get_amount($data->details->charges->conversion_amount,billPayCurrency($data)['sender_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span></li>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header p-3" id="approvedModalLabel">
                <h5 class="modal-title">{{ __("Approved Confirmation") }} ( <span class="fw-bold text-success">{{ get_amount($data->request_amount,billPayCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span> )</h5>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header p-3" id="rejectModalLabel">
                <h5 class="modal-title">{{ __("Rejection Confirmation") }} ( <span class="fw-bold text-danger">{{ get_amount($data->request_amount,billPayCurrency($data)['wallet_currency'],get_wallet_precision($data->creator_wallet->currency)) }}</span> )</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
