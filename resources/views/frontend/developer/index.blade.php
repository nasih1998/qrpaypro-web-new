...
<div class="developer-body-wrapper">
    <div class="developer-main-wrapper">
        <h1 class="heading-title mb-20 text-center">{{ __($intro->value->language->$lang->heading ?? $intro->value->language->$system_default->heading) }}</h1>
        <div class="image-area text-center">
            <img class="w-75" src="{{ get_image(@$intro->value->images->intro_image,'site-section') }}" alt="image">
        </div>
        <p>{{ __($intro->value->language->$lang->details ?? $intro->value->language->$system_default->details) }}</p>
        <h1 class="heading-title mb-20 mt-40">1. {{__("Introduction") }}</h1>
        <p>{{ __($intro->value->language->$lang->intro_details ?? $intro->value->language->$system_default->intro_details) }}</p>
    </div>
